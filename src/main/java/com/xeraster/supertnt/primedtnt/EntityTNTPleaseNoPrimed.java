...
public class EntityTNTPleaseNoPrimed extends Entity {

    private static final DataParameter FUSE = EntityDataManager.createKey(EntityTNTPleaseNoPrimed.class, DataSerializers.VARINT);
    @Nullable
    private EntityLivingBase tntPlacedBy;
    private int fuse;

    public EntityTNTPleaseNoPrimed(World worldIn) {
        super(worldIn);
        this.fuse = 80;
    ...
    public EntityTNTPleaseNoPrimed(World worldIn, double x, double y, double z, EntityLivingBase igniter) {
        this(worldIn);
    ...
    protected void entityInit() {
        this.dataManager.register(EntityTNTPleaseNoPrimed.FUSE, Integer.valueOf(80));
    }
    ...
    private void explode() {
        ...
        for (int x = -5000; x < 5000; ++x) {
            float currentProgress = (float) (x + 5000);
            ...
            System.out.println("on X row number " + x);
    ...
    public void setFuse(int fuseIn) {
        this.dataManager.set(EntityTNTPleaseNoPrimed.FUSE, Integer.valueOf(fuseIn));
        this.fuse = fuseIn;
    }

    public void notifyDataManagerChange(DataParameter key) {
        if (EntityTNTPleaseNoPrimed.FUSE.equals(key)) {
    ...
    public int getFuseDataManager() {
        return ((Integer) this.dataManager.get(EntityTNTPleaseNoPrimed.FUSE)).intValue();
    }
