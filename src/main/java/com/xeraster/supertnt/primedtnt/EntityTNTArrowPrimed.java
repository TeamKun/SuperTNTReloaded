...
public class EntityTNTArrowPrimed extends Entity {

    private static final DataParameter FUSE = EntityDataManager.createKey(EntityTNTArrowPrimed.class, DataSerializers.VARINT);
    @Nullable
    private EntityLivingBase tntPlacedBy;
    private int fuse;

    public EntityTNTArrowPrimed(World worldIn) {
        super(worldIn);
        this.fuse = 80;
    ...
    public EntityTNTArrowPrimed(World worldIn, double x, double y, double z, EntityLivingBase igniter) {
        this(worldIn);
    ...
    protected void entityInit() {
        this.dataManager.register(EntityTNTArrowPrimed.FUSE, Integer.valueOf(80));
    }
    ...
    private void explode() {
        byte numArrowsToSpawn = 50;
        float f = 25.0F;

        for (int i = 0; i < numArrowsToSpawn + 1; ++i) {
            this.arrow();
        }
    ...
    private void arrow() {
        if (!this.world.isRemote) {
            ...
            double x = (double) (-((float) Math.sin((double) f)) * 0.75F);
            double y = Math.abs(Math.random() * 0.75D);
            double z = (double) (-((float) Math.cos((double) f)) * 0.75F);
            EntityTippedArrow entity = new EntityTippedArrow(this.world);

            entity.setLocationAndAngles(this.posX, this.posY, this.posZ, 0.0F, 0.0F);
            entity.motionX = x;
            entity.motionY = y;
            entity.motionZ = z;
            entity.setDamage(30.0D);
    ...
    public void setFuse(int fuseIn) {
        this.dataManager.set(EntityTNTArrowPrimed.FUSE, Integer.valueOf(fuseIn));
        this.fuse = fuseIn;
    }

    public void notifyDataManagerChange(DataParameter key) {
        if (EntityTNTArrowPrimed.FUSE.equals(key)) {
    ...
    public int getFuseDataManager() {
        return ((Integer) this.dataManager.get(EntityTNTArrowPrimed.FUSE)).intValue();
    }
