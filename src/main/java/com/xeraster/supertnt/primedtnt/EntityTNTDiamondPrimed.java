...
public class EntityTNTDiamondPrimed extends Entity {

    private static final DataParameter FUSE = EntityDataManager.createKey(EntityTNTDiamondPrimed.class, DataSerializers.VARINT);
    @Nullable
    private EntityLivingBase tntPlacedBy;
    private int fuse;

    public EntityTNTDiamondPrimed(World worldIn) {
        super(worldIn);
        this.fuse = 80;
    ...
    public EntityTNTDiamondPrimed(World worldIn, double x, double y, double z, EntityLivingBase igniter) {
        this(worldIn);
    ...
    protected void entityInit() {
        this.dataManager.register(EntityTNTDiamondPrimed.FUSE, Integer.valueOf(80));
    }
            ...
            for (int y = -20; y < 20; ++y) {
                for (int z = -20; z < 20; ++z) {
                    ...
                    if (this.world.getBlockState(blockScanPos).getMaterial() != Material.AIR && this.world.getBlockState(blockScanPos).getBlock() != Blocks.BEDROCK && this.world.getBlockState(blockScanPos).getMaterial() != Material.AIR) {
                        this.world.setBlockState(blockScanPos, Blocks.DIAMOND_BLOCK.getDefaultState());
                    }
                }
            }
        }
    ...
    public void setFuse(int fuseIn) {
        this.dataManager.set(EntityTNTDiamondPrimed.FUSE, Integer.valueOf(fuseIn));
        this.fuse = fuseIn;
    }

    public void notifyDataManagerChange(DataParameter key) {
        if (EntityTNTDiamondPrimed.FUSE.equals(key)) {
    ...
    public int getFuseDataManager() {
        return ((Integer) this.dataManager.get(EntityTNTDiamondPrimed.FUSE)).intValue();
    }
