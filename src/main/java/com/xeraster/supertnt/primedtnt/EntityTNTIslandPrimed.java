...
public class EntityTNTIslandPrimed extends Entity {

    private static final DataParameter FUSE = EntityDataManager.createKey(EntityTNTIslandPrimed.class, DataSerializers.VARINT);
    @Nullable
    private EntityLivingBase tntPlacedBy;
    private int fuse;

    public EntityTNTIslandPrimed(World worldIn) {
        super(worldIn);
        this.fuse = 80;
    ...
    public EntityTNTIslandPrimed(World worldIn, double x, double y, double z, EntityLivingBase igniter) {
        this(worldIn);
    ...
    protected void entityInit() {
        this.dataManager.register(EntityTNTIslandPrimed.FUSE, Integer.valueOf(80));
    }
    ...
    private void explode() {
        ...
        for (int x = -40; x < 40; ++x) {
            for (int y = 30; y > -20; --y) {
                for (int z = -35; z < 35; ++z) {
                    IBlockState blockToCopy = this.world.getBlockState(new BlockPos(this.posX + (double) x, this.posY + (double) y, this.posZ + (double) z));

                    this.world.setBlockState(new BlockPos(this.posX + (double) x, this.posY + (double) y + 50.0D, this.posZ + (double) z), blockToCopy);
                }
            }
        }
    ...
    public void setFuse(int fuseIn) {
        this.dataManager.set(EntityTNTIslandPrimed.FUSE, Integer.valueOf(fuseIn));
        this.fuse = fuseIn;
    }

    public void notifyDataManagerChange(DataParameter key) {
        if (EntityTNTIslandPrimed.FUSE.equals(key)) {
    ...
    public int getFuseDataManager() {
        return ((Integer) this.dataManager.get(EntityTNTIslandPrimed.FUSE)).intValue();
    }
